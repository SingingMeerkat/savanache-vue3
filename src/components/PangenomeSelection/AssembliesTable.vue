<template>
  <div>
    ...
    <v-dialog v-model="showModal" max-width="290">
      <v-card>
        ...
        <v-card-text>
          ...
          <v-checkbox v-for="option in heteroticGroups" :key="option.value" v-model="selected" :label="option"
                      :value="option" hide-details></v-checkbox>
        </v-card-text>
    ...
    <va-data-table :columns="headers" :hoverable="true" :items="assembliesTable" @mouseout="myUnRowHover($event)"
      ...
      <template #header(phenotype)="data">
        <div class="row">
          ...
          <va-input v-model="phenotypeFilter" placeholder="Filter..." />
        </div>
      </template>
      ...
      if (sortBy.value !== "" && assembliesToTable.length !== 0) {
        if (directionLastSorted.value === "ascd") {
          ...
          assembliesToTable = assembliesToTable.sort((a, b) =>
            a[sortBy.value].localeCompare(b[sortBy.value], navigator.languages[0] || navigator.language, {
              numeric: true,
          ...
          assembliesToTable = assembliesToTable.sort(
            (a, b) => a[sortBy.value].localeCompare(b[sortBy.value], navigator.languages[0] || navigator.language, {
              numeric: true,
              ignorePunctuation: true
            }) * -1
          );
